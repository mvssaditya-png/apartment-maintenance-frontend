...
export default function PaymentHistoryScreen() {
  ...
  const downloadAndShareExcel = async (url, fileName) => {
    try {
      ...
      if (result.status !== 200) {
        Alert.alert(t("common.error"), t("paymentHistory.exportFailed"));
        return;
      }
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(result.uri, {
          ...
          dialogTitle: t("paymentHistory.shareStatement"),
          UTI: "com.microsoft.excel.xlsx",
        });
      } else {
        Alert.alert(t("paymentHistory.downloaded"), t("paymentHistory.exportSuccess"));
      }
    } catch (error) {
  ...
  const handleExportSelectedFlat = async () => {
    const flatId = selectedFlat?.flatId || user?.flatId || user?.flat_id;

    if (!flatId) {
      Alert.alert(t("common.error"), t("paymentHistory.flatNotSelected"));
      return;
    }
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["bottom"]}>
      ...
      >
        <Text style={styles.heading}>{t("paymentHistory.title")}</Text>

        <Text style={styles.subtitle}>{t("paymentHistory.subtitle")}</Text>
            ...
            <View style={styles.summaryTextBlock}>
              <Text style={styles.summaryLabel}>
                ...
                {isAdminOrCashier
                  ...
                  : user?.flatNumber || user?.flat_number || "-"}
              </Text>
              ...
              <Text style={styles.summarySubText}>
                {Number(currentBalance || 0) < 0
                  ? t("paymentHistory.pendingBalance")
                  : t("paymentHistory.availableBalance")}
              </Text>
            </View>
        ...
        <View style={styles.exportRow}>
          <AppButton
            title={t("paymentHistory.exportSelectedFlat")}
          ...
          {isAdminOrCashier && (
            <AppButton
              title={t("paymentHistory.exportAllFlats")}
              variant="success"
        ...
        {statementLoading ? (
          <AppCard>
            <View style={styles.statementLoader}>
              ...
              <Text style={styles.loaderText}>
                {t("paymentHistory.loadingStatement")}
              </Text>
            </View>
          </AppCard>
        ) : statement.length === 0 ? (
          <AppCard>
            <EmptyState
              ...
              title={t("paymentHistory.noHistory")}
              subtitle={t("paymentHistory.noEntries")}
            />
          </AppCard>
        ) : (
    ...
    <AppCard style={styles.statementCard}>
      ...
      <View style={styles.balanceRow}>
        <Text style={styles.balanceLabel}>
          {balanceAfter < 0
            ? t("paymentHistory.pendingAfter")
            : t("paymentHistory.balanceAfter")}
        </Text>
      ...
      {isCredit && item.receiptPdfUrl ? (
        ...
        >
          ...
          <Text style={styles.receiptButtonText}>
            {t("paymentHistory.viewReceiptPdf")}
          </Text>
        </TouchableOpacity>
      ) : null}
    </AppCard>
